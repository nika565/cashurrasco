...
const TelaHistorico = ({ route, navigation }) => {
    ...
    return (
      <View style={estilos.card}>
        ...
        <Text style={estilos.btnTexto}>
          ...
          {item.qtdHomens + item.qtdMulheres + item.qtdCriancas}
        </Text>
        ...
        >
          <Text style={estilos.btnTexto}>Ver Mais</Text>
        </TouchableOpacity>
      </View>
    );
  };
  ...
  return (
    <View style={{ padding: 10, backgroundColor: "#260101", height: "100%", flex: 1 }}>
      {error ? ( // Verifica se há uma mensagem de erro
        <Text style={estilos.titulo}>Mensagem de Erro: {error}</Text>
      ) : (
        <View style={{ backgroundColor: "#260101", flex: 1 }}>
          <Text style={estilos.titulo}>Histórico de eventos</Text>

          <TouchableOpacity style={estilos.btnRecarregar} onPress={async () => {
            const dadosApi = await todosEventos(route.params.id);
            ...
            <Text style={estilos.btnTexto}>Recarregar</Text>
          </TouchableOpacity>
